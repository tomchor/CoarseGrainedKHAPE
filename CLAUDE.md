...
| `03_energy_transfer.py` | Cross-scale APE flux Π_A and the APE↔KE exchange, computed offline (Π_K is online and read by `04`; see Data flow) |
...
`sweep*` scripts are the sweep variant (parameter sweep over filter scales; `submit_sweep.sh EXTENSION=odd` runs it with b oddly reflected past the walls into `_odd` files, and a run over a subset of scales, e.g. the wall-extension test `submit_extension_test.sh` submits, tags its files `_l<scales>` so it never replaces the full sweep; `sweep2` writes only the `∫` integrals, which is all its readers use, unless given `--keep-fields`): `sweep1_filter_fields.py` filters, `sweep2_energy_transfer.py` computes transfer, `sweep3_plot_transfer_spectrum.py` plots spectra.
...
All post-processing scripts accept `--filename`, `--filter-scales`, `--n-workers`, `--fixed-reference` via argparse; `03`–`06` and `sweep2` additionally accept `--reference {filtered,true}` (see *Filtered-reference scale decomposition*). Output goes to `postprocessing/output/`.

### Data flow between pipeline steps

The sorted density (`*_sorted_density.nc`) produced by step 02 is reused by steps 03 and 05, avoiding redundant sorts. Its column heights are the padded grid's own, so 03, 04, 05 and S5 refuse a sorted file whose `n_pad_z` or `z_extension` differs from the grid they loaded (`check_same_padded_grid`), and every step after 01 raises when 01's filtered file is missing instead of falling back to the Nz//2 padding. 05 reuses its checkpoints only when they were written for the same run: each is stamped with a hash of the loaded dataset's attributes (the simulation's parameters and creation date, `n_pad_z`, `z_extension`) and time axis, and one that differs is deleted and recomputed. When `--fixed-reference` is used, output files are suffixed `_fixed_ref`. **The sweep does not share it.** The column's z✶ are the padded grid's own heights, and the sweep pads to 4σ of ℓ=20 while 02 pads to the budget scales, so 02's column belongs to a different grid (at Nz=2048: 1024 cells per side against the sweep's 2784) and reading it would shift every displacement with nothing downstream to reveal it. `sweep2_energy_transfer.py --fixed-reference` therefore builds its own frozen column, by sorting t=0 on the grid it loaded and broadcasting that row over the time axis — one sort, not one per output, and bit-identical to 02's whenever the two paddings do coincide. `--reference true` adds `_trueref` (after any `_fixed_ref`) to the outputs and checkpoints of 03–06 and sweep2, so the two references never overwrite each other (02's sorted density does not depend on the reference and is shared); 03 and 04 record the reference as the `ape_reference` attribute, and 05 refuses 03/04 output whose attribute disagrees with its own flag. Every script that reads those outputs (06, `plot2`–`plot4`, `anim1_panels`, `sweep3`, `compare_extension` and the `S*` scripts) takes `--reference` too, and tags its figures the same way.

The cross-scale KE transfer **Π_K and the SFS KE dissipation ε_Kˢ are computed online** by the Julia simulation (`kelvin_helmholtz_instability.jl`, output as `Π_K_ℓ<ℓ>` / `ε_Ks_ℓ<ℓ>`). To avoid recomputing them offline, `03_energy_transfer.py` runs with `include_pi_k=False` (Π_A + exchange only) and `04_sfs_ke_budget.py` reads Π_K and ε_Kˢ directly from the simulation output (it still computes the SFS-KE density/tendency and the APE↔KE exchange offline — the exchange needs the sorted reference state). Π_K and ε_Kˢ are built from velocities alone, so they do not depend on the reference state and `04` reads them under every variant; the budget filter scales (`--filter-scales`) must therefore be a subset of the simulation's online `filter_ℓs` (set by `--filter_ls`, default `(1, 7)`), and the offline recomputes are kept under `validation/` for cross-checking (`inv02` for Π_K, `inv05` for ε_Kˢ). Every APE term is computed offline, for both `--reference` values and both reference-profile variants: `03` computes Π_A, and `05` computes ε_Aˢ, the local APE fields, the tendency and Rˢ, all against the offline ⟨ρ_*⟩. The simulation's own APE set (`Π_A_ℓ<ℓ>`, `ε_As_ℓ<ℓ>` and the rest, written under `--save_sorted`) is the cross-check that `inv08`, `inv09` and `inv10` make and the source of the online panels animation; the budget does not read it, because its ⟨b✶⟩ is built on a coarse column and is not the construction the offline terms use (`filtered_reference_decisions.md` §6). That choice has a cost: the online ε_Aˢ pairs its gradients on the cell faces while the offline one takes centred differences, which at Nz=128 put the APE residual at 18.1% of the dominant term against 2.0% (ℓ=1); at Nz=1024 the offline path moves it from 0.185% to 0.477% (ℓ=1) and from 0.287% to 0.320% (ℓ=7). `--save_sorted` is on by default in the submission scripts for the cross-check; `SAVE_SORTED=0` gives smaller output and changes no budget number, and `VALIDATE=1` turns it back on.
...
- **The online APE terms are the filtered-reference ones, and the only ones.** `Π_A_ℓ<ℓ>` / `ε_As_ℓ<ℓ>` are built against ⟨b✶⟩; the simulation no longer writes an unfiltered counterpart. The offline budget reads neither: `03` and `05` compute Π_A and ε_Aˢ for both `--reference` values, against the exact offline ⟨ρ_*⟩, and the online set is the cross-check. The online ε_Aˢ is the better discretised of the two, since the offline `calculate_ape_dissipation` takes centred differences of a quantity quadratic in gradients; measured against the online field, that error was essentially the whole offline residual at Nz=128 (corr +0.995; offline/online ε_Aˢ drifts 0.97 → 0.78 as the interface reaches the grid scale). At Nz=1024 it is under half a percent of the dominant term, which is why the budget takes the one exact ⟨ρ_*⟩ over the online discretisation.
- **`⟨ρ_*⟩` is scale-dependent**, so it is rebuilt inside each filter-scale loop rather than once per run.
- **Cost is essentially unchanged.** The construction is the same lookup and the same cumulative integral against a different profile; the only new work is one 1D convolution of the profile per timestep. The expensive full-field sort in `02` is untouched and still shared. A *frozen* profile is filtered once and broadcast rather than once per output (callers pass `frozen=`, checked against the first and last rows, rather than the function comparing every row), since `sorted_timeseries(fixed_reference=True)` repeats one row — bit-identical, and 12–45× faster at Nz=128.

- **The z padding is sized to the widest filter.** `S̃ = Ē_A − L̃` is an identity only where `filter(z) = z`, which fails within one stencil of an array boundary. `required_pad_margin` asks for 4σ of margin; `01`/`sweep1` pad with it and record it on the filtered-fields file, and `02`–`05`/`sweep2` read it back so every step sees the same padded grid. The old fixed `Nz//2` gives a margin of Lz/2 = 12.5, which is just enough for ℓ = 7 (4σ = 11.89) and fails from ℓ ≈ 7.7 — the next scale in the sweep's `geomspace(0.02, 20, 30)`, whose top end needs 33.97. A run at ℓ ≤ 7 is unchanged, bit-identical. **This fixes only the stencil-truncation half of the large-ℓ problem:** a filter whose σ is a third of the domain also flattens ⟨b_*⟩ until its inverse z̃_* is poorly conditioned, and padding cannot fix that. The profile filter itself is an FFT convolution of the column, O(N log N) in its length, so wider padding costs little there.

`test_filtered_reference.py` pins this down on the synthetic field of `test_jensen.py`: S̃ and L̃ are non-negative, both vanish for a fluid at rest while L measured against the unfiltered ρ_* does not, and the remainder construction still breaks on the same field. (L̃ + S̃ = Ē_A is not tested: S̃ is defined that way.) Measured interior min/rms for S̃ is +8.6e-06, +6.9e-05 and +2.0e-03 at ℓ = 2, 4 and 8 cells, against −0.69, −2.31 and −2.67 for the remainder, which is negative over 15–48% of the domain.

**Edge caveat.** S̃ is computed as Ē_A − L̃, an identity that needs filter(z) = z. `GaussianFilter` extends the bounded z axis with its edge value, so within a stencil of a wall the kernel is one-sided and the identity picks up a spurious τ(z, b). On `test_jensen.py`'s shorter column, whose stratification reaches the walls, every violation lives in that band and nowhere else, which `test_filtered_reference.py::test_edge_cells_are_the_only_place_the_identity_fails` asserts; it also fails if that column stops violating at all, or if `EDGE_BAND_SIGMAS` grows past the kernel's 4σ reach. The production pipeline does not meet this, because `_pad_domain_in_z` pads the domain with edge values at load time, by at least Nz//2 cells each side and at least 4σ of the widest filter, putting the physical domain far from the filter's edge.

**The online APE set, a cross-check.** Under `--save_sorted` the simulation writes the filtered-reference set per scale, which the offline budget does not read (see the Data flow section) — `E_as_ℓ<ℓ>` (S̃), `Π_A_ℓ<ℓ>`, `ε_As_ℓ<ℓ>`, `R_s_ℓ<ℓ>` and the S̃ tendency `dEas_dt_ℓ<ℓ>` — under the plain names, the unfiltered set having been removed entirely so that nothing downstream can read the superseded formulation by accident. L̃ is built only to form S̃; neither it nor Υ̃ is written, since nothing read them. (`wb_rs_ℓ<ℓ>`, the APE→KE conversion, is filter(w(b − b✶)) − w̄(b̄ − ⟨b✶⟩): its resolved half is on ⟨b✶⟩ like every other term, matching `04`. Upstream's `SubFilterAvailablePotentialToKineticEnergyConversion` reads one profile for both halves, so neither `method=lookup` nor `method=lookup_flt` gives this split; the simulation adds the filtered conversion against b✶ and subtracts it against ⟨b✶⟩, which moves the pointwise field and leaves every integral as it was, since ∫w̄ f(z) dV = 0. It is a reversible exchange, so **neither half of the filtered/sub-filter separation is sign-definite** and no positivity test applies to it — unlike ε_Kˢ or S̃. It is also load-bearing: `inv10` reads `wb_rs_ℓ<ℓ>_int` in *both* budgets, where at Nz=128/Re=262 it is the largest KE term at ℓ=1, and `plot_kelvin_helmholtz_instability.jl` discovers its panel scales from the `wb_rs_ℓ*` names, so removing it deletes every panels animation. The offline pipeline ignores it — `04` builds its own exchange and `05` reads `04`'s — which makes it look unused.) It needed no Oceanostics change: `ProfileLookup` accepts an external profile as a pair of `Field`s and refreshes it on every `compute!` (`refresh_profile_source!(p::Field, time) = compute_at!(p, time)`), supplying one skips the O(N log N) sort, and the two-argument constructors `AvailablePotentialEnergy(model, z✶)` / `FilteredAvailablePotentialEnergy(model, z✶ˡ)` / `AvailablePotentialEnergyCrossScaleFlux(model, filter, z✶ˡ)` let the two halves carry different profiles, composed in the simulation script exactly as `R_s = Field(gf(R_full)) - R_l` already is. Three things that cost real time to learn and are easy to undo by accident:
...
- The online diagnostics come from the Oceanostics 0.21.0 release; the Manifest tracks no Oceanostics branch, only the Oceananigans branch `tc/time-derivatives3` that provides `TimeDerivative`. The filtered-reference APE terms are composed in the simulation script from those diagnostics, a filtered full-field term against b✶ minus a filtered-state term against ⟨b✶⟩, and the repository has its own code where Oceanostics has none: `ReferenceTendencyCorrection` in `online_diagnostics.jl`, the coarse-column kernels that build ⟨b✶⟩ (`coarse_column`, `coarse_filter`, `_interp_from_coarse_ccc`), and `flatten`. Oceanostics provides `GaussianFilter`, `StrainRateTensor`, `subfilter_stress_tensor`, `KineticEnergyCrossScaleFlux`, `SubFilterKineticEnergyDissipationRate`, the whole `BackgroundPotentialEnergyEquation`/`AvailablePotentialEnergyEquation` reference-state machinery, `AvailablePotentialEnergyCrossScaleFlux` (PR #295), `SubFilterAvailablePotentialToKineticEnergyConversion` (PR #301), the `FilteredAvailablePotentialEnergyEquation` family, and `AvailablePotentialEnergyDisplacementPotential`/`AvailablePotentialEnergyDissipationRate` (0.20.0 renamed the first from `BuoyancyDisplacementPotential`). Note 0.19.0 renamed `KineticEnergyBuoyancyProduction` → `PotentialToKineticEnergyConversion`; this repo does not use it.
...
- This is the online counterpart of the offline sort in `02_sort_density.py`. The offline pipeline sorts the *z-padded* domain (`load_dataset_and_grid` pads it with edge values at load time) while the online sort sees only the true domain, so the two do not sort the same field; `inv06_compare_sorted_profiles.py` quantifies that, running the offline sort both padded and unpadded to separate the padding effect from any genuine online-vs-offline difference.

#### Online sub-filter APE dissipation ε_Aˢ (also gated behind `--save_sorted`)
Per online filter scale, the simulation builds the diffusive sink of the sub-filter APE budget from Oceanostics' `AvailablePotentialEnergyDissipationRate`, measured against b✶ and then filtered, and `FilteredAvailablePotentialEnergyDissipationRate`, measured against ⟨b✶⟩. The single-profile `SubFilterAvailablePotentialEnergyDissipationRate` (Oceanostics PR #293) is not used, since it measures both halves against one profile:
- **ε_Aˢ = filter(ε_A) − ε̃ˡ**, with **ε_A = κ ∂ᵢb ∂ᵢΥ** the total APE dissipation, **Υ = z✶ − z** the buoyancy displacement potential, **ε̃ˡ = −q̄ᵢ ∂ᵢΥ̃**, **q̄ᵢ = filter(κ ∂ᵢb)** and **Υ̃ = z̃✶(b̄) − z**, the filtered buoyancy's reference height looked up in ⟨b✶⟩. Output as `ε_As_ℓ<ℓ>` and `ε_As_ℓ<ℓ>_int`, matching the `Π_K_ℓ<ℓ>` / `ε_Ks_ℓ<ℓ>` naming. `05_sfs_ape_budget.py` does not read them: it computes ε_Aˢ offline with `calculate_sfs_ape_dissipation` (see the Data flow section), and `inv08` and `inv10` use the online field as the cross-check.
- **Only ε_Aˢ is written.** Υ and the total ε_A are Oceanostics' `BuoyancyDisplacementPotential` and `AvailablePotentialEnergyDissipationRate` (both developed here and merged upstream in PR #276); the script builds ε_A and Υ̃ only to form ε_Aˢ; neither is a budget term, so neither is written.
...
- **It tracks the current sort.** The online ε_Aˢ is tied to the simulation's own sort of the current buoyancy, so it corresponds to the time-varying reference only and has no counterpart under `--fixed-reference`. `05_sfs_ape_budget.py` computes ε_Aˢ offline in both variants; see the Data flow section.
- **Two profiles, one sort.** The full-field half is measured against b✶ and the filtered-state half against ⟨b✶⟩: that is the filtered-reference split, so the two halves carry different profiles on purpose. Both come from the one `VerticalSort` column (⟨b✶⟩ is that column's b✶ filtered on the coarse grid and interpolated back), so every filter scale still shares one sort. Offline, `local_potential_energies_timeseries` does the same: the filtered density's reference height is a nearest-density lookup in ⟨ρ_*⟩, the full field's in ρ_*.
- **The second term differs from the offline one by construction.** Online ε_Aˡ contracts the *filtered flux* `filter(κ∂ᵢb)` with ∇Υ̃; offline term 2 rebuilds the flux from the filtered density as `κ∇ρ̄`. Filtering the flux rather than recomputing it is what makes ε_Aˡ the sink of the filtered-state budget when κ varies in space. For the constant κ used here the two agree in the interior (filtering and differencing are both convolutions on a uniform grid, so they commute) and part ways only against the walls, where the filter's edge extension and the derivative's one-sided stencil disagree.
- `inv08_compare_sfs_ape_dissipation.py` measures what is left of that, plus a conservative-vs-centered discretization gap: the online form pairs its two factors on the face where both differences live and interpolates the *product* to the center, while the offline `calculate_gradient` takes centered derivatives at the center and multiplies *those*. Being a difference of two comparable quantities, ε_Aˢ could have amplified that the way ε_Kˢ does; measured at Nz=192/Re=262, before the filtered reference, it does not, landing at 7.8e-02 (field) and 1.4e-01 (∫ε_Aˢ dV) for ℓ=1, and 4.0e-02 / 1.1e-01 for ℓ=7.
...
- **The online Eₐˢ is S̃, and is non-negative.** Against the unfiltered b✶ the remainder filter(eₐ) − eₐˡ has no fixed sign under an x–z filter: Jensen needs the filter to act on b alone (`test_jensen.py`). Against ⟨b✶⟩ it is S̃ of the filtered-reference split, non-negative up to the nearest-slot lookup, which `test_jensen.py`'s online test asserts to 1e-3 of rms.
- Measured at Nz=1024 on 2026-09-17 with the filtered-reference set (inv10, posted in PR #68), residual/min(terms) is 0.12% (ℓ=1) and 0.56% (ℓ=7) for KE, and 0.51% and 2.1% for APE. The earlier figures at Nz=192/Re=262 (KE 8.8% and 0.53%, APE 5.5% and 1.9%) were for the unfiltered formulation.
