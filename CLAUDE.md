...
- The **sub-filter** dissipation ε_Aˢ = filter(κ∂ᵢρ∂ᵢΥ) − κ∂ᵢρ̄∂ᵢΥˡ is deliberately *not* computed online: it needs a second reference state sorted from the filtered buoyancy. It stays in `05_sfs_ape_budget.py`, which now builds both of its terms from the offline `calculate_ape_dissipation` — the same expression `inv08` validates against.
