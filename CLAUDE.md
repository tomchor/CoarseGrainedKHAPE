...
- **Only ε_Aˢ is written.** Υ and the total ε_A are Oceanostics' `BuoyancyDisplacementPotential` and `AvailablePotentialEnergyDissipationRate` (both developed here and merged upstream in PR #276); the sub-filter diagnostic builds them internally, but neither is a budget term, so neither is emitted. An earlier revision of this repo did emit them, validated by an `inv08` script — both are gone.
...
- **Units convention.** The offline pipeline is written in density: `Υ = g(z − z✶(ρ))/ρ₀`, Eq. (7) of Wenegrat, Chor & Barkan (2026). A Boussinesq `BuoyancyTracer` model has no g or ρ₀, so the online Υ is the buoyancy form `z✶ − z`, related by `Υ_density = −(g/ρ₀) Υ_buoyancy`. Since `b = g(ρ₀−ρ)/ρ₀` makes `∇ρ = −(ρ₀/g)∇b`, both sign flips cancel in the contraction, so **ε_A — and hence ε_Aˢ — is the same number in either form** and `inv09` compares them directly.
