...
- `b✶`, `z✶` (`OneDimensionalSort`) — the sorted column, on its own N = Nx·Ny·Nz vertical axis (one slab per model cell), in `output/khi_Nz<Nz>_Ri<Ri>_sorted.nc`. **It needs its own file.** A single `NetCDFWriter` can hold both grids (the upstream `lock_release.jl` example does this), but as soon as a writer sees two grids Oceananigans suffixes every dimension (`x_caa` → `x_caa_grid1`) and renames the metadata groups (`underlying_grid_reconstruction_kwargs` → `grid_1_...`). That renames the axes of every other field in the file and breaks 01–06, the sweep, and the `inv0*` scripts, all of which read `x_caa`/`z_aac` and that group.
...
- The API was renamed upstream: `sorted_reference_height` → `reference_height`, `sorted_buoyancy` → `reference_buoyancy`.
