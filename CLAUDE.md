...
**Now computed online too.** Under `--save_sorted` the simulation writes the filtered-reference set per scale alongside the unfiltered one, suffixed `_fref`: `L_fref` (L̃), `E_as_fref` (S̃), `Υ_fref`, `Π_A_fref`, `ε_As_fref`, `R_s_fref` and the S̃ tendency. It needed no Oceanostics change: `ProfileLookup` accepts an external profile as a pair of `Field`s and refreshes it on every `compute!` (`refresh_profile_source!(p::Field, time) = compute_at!(p, time)`), supplying one skips the O(N log N) sort, and the two-argument constructors `AvailablePotentialEnergy(model, z✶)` / `FilteredAvailablePotentialEnergy(model, z✶ˡ)` / `AvailablePotentialEnergyCrossScaleFlux(model, filter, z✶ˡ)` let the two halves carry different profiles, composed in the simulation script exactly as `R_s = Field(gf(R_full)) - R_l` already is. Three things that cost real time to learn and are easy to undo by accident:
...
- **The `_fref` set stays out of the 2D panels writer**, which has no use for it and is the largest output tuple.
