...
#### Online sub-filter APE dissipation ε_Aˢ (written by default)
`SubFilterAvailablePotentialEnergyDissipationRate` (Oceanostics PR #293) computes, per online filter scale, the diffusive sink of the sub-filter APE budget:
...
- **Tendencies** are written both as full fields (`dKs_dt_ℓ<ℓ>`, `dEas_dt_ℓ<ℓ>`) and as tendencies of the integrals (`..._int`, the budget-closure form). They come from Oceananigans' `TimeDerivative`, which is **callback-driven**: it advances only when it is *called*, and reading it (`compute!` included) leaves it at whatever its callback last computed. An output writer registers one `Callback` per bare `TimeDerivative` among its outputs, on `PrecedingIterations(writer.schedule)`, so the derivative is updated at the iteration before the writer actuates and again at the actuation, and the written value is the backward difference over that single timestep, (aⁿ − aⁿ⁻¹)/Δt labelled tⁿ. No record is left pending, so no output is ever NaN. R needs its callback registered by hand: ∂ₜb✶ is reached only through `ReferenceTendencyState`, not as a writer output, so `kelvin_helmholtz_instability.jl` gives it `Callback(∂ₜb✶, PrecedingIterations(...))` on the 3D writer's schedule where the writers are built. Callbacks run before the writers within a time step, so every Rˢ is fetched with ∂ₜb✶ already advanced. `online_diagnostics.jl` also adds the `actuates_next_iteration` method for `ConsecutiveIterations` that upstream lacks; without it `PrecedingIterations` cannot anticipate the 3D writer's actuation and falls back to actuating every iteration, which would re-sort the column at every timestep instead of three times per output. (Before Oceananigans 0.113 this all worked differently: `TimeDerivative` was a *deferred* output the writer evaluated twice and back-filled, a `deferred_output(::ReferenceTendencyState) = true` method propagated the deferral through R, the difference was forward rather than backward, and the last record of a run held NaN.)
...
- **The first output pair is not a budget statement.** Its derivatives span the initialisation transient where `∫Eₐˢ` climbs from zero to its working value. `inv10` drops both records (`--skip`, default 2); every later record is kept, the last one included.
