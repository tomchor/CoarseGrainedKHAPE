...
- Every online diagnostic used here comes from Oceanostics, with no bespoke diagnostic code left in this repo. The sub-filter APE dissipation ε_Aˢ needs `SubFilterAvailablePotentialEnergyEquation`, which is still on the `tc/subfilter-ape` branch (Oceanostics PR #293), so `Manifest.toml` pins that branch until it is released; everything else is in the released 0.19.0, which provides `GaussianFilter`, `StrainRateTensor`, `subfilter_stress_tensor`, `KineticEnergyCrossScaleFlux`, `SubFilterKineticEnergyDissipationRate`, the whole `BackgroundPotentialEnergyEquation`/`AvailablePotentialEnergyEquation` reference-state machinery, and `BuoyancyDisplacementPotential`/`AvailablePotentialEnergyDissipationRate`. Note 0.19.0 renamed `KineticEnergyBuoyancyProduction` → `PotentialToKineticEnergyConversion`; this repo does not use it.
...
#### Online APE dissipation: Υ and ε_A (also gated behind `--save_sorted`)
The buoyancy displacement potential and the **total** APE dissipation rate are computed online too. Both were developed in this repo and merged into Oceanostics 0.19.0 (PR #276), so they now come straight from `AvailablePotentialEnergyEquation` and take the same interface `AvailablePotentialEnergy`/`BackgroundPotentialEnergy` use — a keyword form that builds its own z✶ from a `method`, and a two-argument form taking a z✶ you already have:
- `BuoyancyDisplacementPotential(model[, z✶])` → **Υ = z✶ − z** [m], and `AvailablePotentialEnergyDissipationRate(model[, z✶]; upsilon)` → **ε_A = κ ∂ᵢb ∂ᵢΥ** [m² s⁻³]. Output as `Υ`, `ε_A` and `∫ε_A`.
- **Units convention, and the one place the online and offline forms differ.** The offline pipeline is written in density: `Υ = g(z − z✶(ρ))/ρ₀`, Eq. (7) of Wenegrat, Chor & Barkan (2026). A Boussinesq `BuoyancyTracer` model has no g or ρ₀, so the online Υ is the buoyancy form `z✶ − z`, related by `Υ_density = −(g/ρ₀) Υ_buoyancy`. Since `b = g(ρ₀−ρ)/ρ₀` makes `∇ρ = −(ρ₀/g)∇b`, both sign flips cancel in the contraction and **ε_A is the same number in either form** — only Υ itself carries the rescaling. `inv08` converts before comparing.
- ε_A is the sink of the local APE equation (the paper's Eqs. 11 and 14, where it enters as −ε_A). With `∂Eₐ/∂b = z✶ − z = Υ`, the diffusive part of `DEₐ/Dt` is `Υκ∇²b = ∇·(κΥ∇b) − κ∇Υ·∇b`. Written out, `ε_A = κ[(∂z✶/∂b)|∇b|² − ∂b/∂z]`: the Winters diapycnal mixing rate less the diffusion the reference state undergoes on its own. The two cancel exactly for a statically stable, horizontally uniform stratification (z✶ = z), so ε_A is **not** sign-definite pointwise the way a `κ|∇b|²` would be — it measures only the APE actually lost.
- `κ ∂ᵢb` comes from the closure's own `diffusive_flux_*` rather than a diffusivity passed in, in the same conservative form Oceanostics' `TracerVarianceDissipationRate` uses for χ: each product is formed on the face where both factors live and only then interpolated to the center, so a no-flux boundary contributes nothing. That also means the buoyancy has to be a tracer the closure diffuses — `BuoyancyTracer` only, enforced with an error.
- Both hang off the **`HeavisideIntegral`** z✶ the `--save_sorted` block already computes, so they cost no extra sort. That is also the method Oceanostics defaults to for these two (unlike the rest of the module, which defaults to `ThreeDimensionalSort`): every use of Υ differentiates it, and only Winters Eq. (11) makes z✶ a function of buoyancy alone, so tied cells share one reference height instead of taking consecutive slots (which `ThreeDimensionalSort` does, spreading z✶ over the depth a tied run fills — harmless in a volume integral, grid-scale noise in ∇Υ). Υ is materialized as a `Field` so ε_A can difference it and the two share one computation.
- The **sub-filter** dissipation ε_Aˢ is computed online too, per filter scale — see the section below.
- Two things make the online and offline numbers differ, both quantified by `inv08` (measured at Nz=192, Re=262 — CI's Reynolds number):
  - **Ties.** The offline `z₀` is a nearest-density lookup, which lands on a tied run's *bottom* slot where the online `HeavisideIntegral` z✶ takes its mid-height. An exactly horizontally uniform z level is one tied run of Nx·Ny cells spanning one cell height, so the two Υ differ there by the constant `Δz/2 − Δz/(2·Nx·Ny)`. At t=0 that is every level and the mean difference lands on that offset exactly; once the perturbation and advection break the symmetry it survives only in the thin bands against the walls where the initial tanh saturated to roundoff, and the mid-run Υ metric drops to 6e-03. `inv08` prints the mean difference next to the predicted offset so it is clear which regime a snapshot is in. Being a constant it drops out of ∇Υ, so ε_A is unaffected.
  - **Discretization**, and it is one-sided. The online form pairs the two factors on the face where both differences live and interpolates the *product* to the center; the offline `calculate_gradient` takes centered derivatives at the center and multiplies *those*, which filters out exactly the grid-scale correlation the product is made of. So the offline ∫ε_A runs low — by ~16% at Nz=192, concentrated in the late strongly-mixing phase where the interface is grid-scale sharp, and shrinking with resolution. Measured: 6e-02 (ε_A field), 1.2e-01 (∫ε_A). The online kernel was separately checked to reproduce the conservative form it intends to compute to machine precision (3.6e-16), so this is the offline discretization, not an implementation gap.
...
- **ε_Aˢ = filter(ε_A) − ε_Aˡ**, with **ε_Aˡ = −q̄ᵢ ∂ᵢΥˡ**, **q̄ᵢ = filter(κ ∂ᵢb)** and **Υˡ = z✶(b̄) − z**. Output as `ε_As_ℓ<ℓ>` and `ε_As_ℓ<ℓ>_int`, matching the `Π_K_ℓ<ℓ>` / `ε_Ks_ℓ<ℓ>` naming. `05_sfs_ape_budget.py` reads them back instead of calling `calculate_sfs_ape_dissipation`, exactly as `04` does for the KE pair.
...
- `inv09_compare_sfs_ape_dissipation.py` measures what is left of that, plus the conservative-vs-centered discretization gap `inv08` already quantifies for the total ε_A. Being a difference of two comparable quantities, ε_Aˢ amplifies that gap in relative terms, the same way ε_Kˢ does on the KE side.
- The offline expression stays in `calculate_sfs_ape_dissipation` (`aux01_pe_functions.py`) as the reference `inv09` compares against, in the same way the offline Π_K and ε_Kˢ paths live on under `validation/`.
