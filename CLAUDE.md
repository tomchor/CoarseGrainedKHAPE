...
- `online_diagnostics.jl` -- the only two budget terms Oceanostics does not provide: `SubFilterAvailablePotentialToKineticEnergyConversion` (τ(w, b_r), the sub-filter half of the split whose filtered half is upstream's `FilteredAvailablePotentialToKineticEnergyConversion`) and `ReferenceTendencyCorrection` (R = ∫_{z✶}^{z} ∂ₜb✶ dz̃)
...
- **Tendencies** come from Oceananigans' `TimeDerivative`. Listing one as a writer output registers a callback on `IterationInterval(1)`, so the difference spans a single model timestep and is centred at tⁿ − Δt/2, effectively at tⁿ where every other term sits. Differencing on the writer's own schedule would centre it half an output interval early and would not close.
...
- **The first output pair is not a budget statement.** A `TimeDerivative` is zero until its operand has been evaluated twice, and the second output's derivative spans the initialisation transient where `∫Eₐˢ` climbs from zero to its working value. `inv10` drops both (`--skip`, default 2).
