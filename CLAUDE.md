...
- `online_ape_dissipation.jl` -- `BuoyancyDisplacementPotential` (Υ) and `AvailablePotentialEnergyDissipationRate` (ε_A), the two diagnostics not yet in Oceanostics, written against the same interface as its `AvailablePotentialEnergy`/`BackgroundPotentialEnergy` (see "Online APE dissipation" below)
...
#### Online APE dissipation: Υ and ε_A (`online_ape_dissipation.jl`, also gated behind `--save_sorted`)
The buoyancy displacement potential and the **total** APE dissipation rate are computed online too. Neither is in Oceanostics yet, so they live in `online_ape_dissipation.jl` at the repo root, written against the same interface `AvailablePotentialEnergy`/`BackgroundPotentialEnergy` use — a keyword form that builds its own z✶ from a `method`, and a two-argument form taking a z✶ you already have:
...
- Both hang off the **`HeavisideIntegral`** z✶ the `--save_sorted` block already computes, so they cost no extra sort. That is also the method to want here: every use of Υ differentiates it, and only Winters Eq. (11) makes z✶ a function of buoyancy alone, so tied cells share one reference height instead of taking consecutive slots (which `ThreeDimensionalSort` does, spreading z✶ over the depth a tied run fills — harmless in a volume integral, grid-scale noise in ∇Υ). Υ is materialized as a `Field` so ε_A can difference it and the two share one computation.
