...
- `Π_K_ℓ<ℓ>`, `ε_Ks_ℓ<ℓ>` and the sub-filter kinetic energy `K_s_ℓ<ℓ>` = ½τⁱⁱ (and their volume integrals) are always written; `04_sfs_ke_budget.py` reads Π_K and ε_Kˢ back, while Kˢ is emitted for validation (`inv09`) and still computed offline by the budget, which needs its tendency anyway. the individual S̄/τ components (`S11/S33/S13_ℓ<ℓ>`, `tau11/tau33/tau13_ℓ<ℓ>`) are gated behind `--save_tensors` and consumed only by `postprocessing/validation/`.
...
- The sub-filter APE itself, **Eₐˢ = filter(eₐ) − eₐˡ** (`SubFilterAvailablePotentialEnergy`, output as `E_as_ℓ<ℓ>`), is emitted alongside, built from the same two reference heights and sharing the same filter and column. It is the energy whose budget ε_Aˢ is the sink of. `05_sfs_ape_budget.py` still builds its own `subfilter_local_ape` offline, since it needs the tendency; `inv10` checks the online field against it.
