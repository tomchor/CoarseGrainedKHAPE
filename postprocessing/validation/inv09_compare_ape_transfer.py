#!/usr/bin/env python
"""
Compare the online (Oceanostics, simulation-time) cross-scale APE flux Π_A against the offline (Python
post-processing) one, at every filter scale.
...
Three differences between the two, none of them errors:
  ...
  * **Units.** The offline form contracts the density flux filter(ρuᵢ) - ρ̄ūᵢ with ∇Υ in the paper's
    density convention, Υ = g(z - z✶(ρ))/ρ₀. The online one is the buoyancy form throughout. Since
    b = g(ρ₀-ρ)/ρ₀ flips the sign of both factors, the two cancel in the contraction and Π_A is the
    *same number* either way, so the two are compared directly.

  * **Discretization.** Online, each τᵢ and ∂ᵢΥˡ is interpolated to the cell center and multiplied
    there; offline, `calculate_gradient` differentiates at the center with `xr.differentiate` and
    multiplies those. The two agree to second order on smooth fields and part ways where Υˡ is
    grid-scale rough.

  * **Ties.** The online z✶ comes from a `ProfileLookup`, which puts a run of equal buoyancy at the
    mid-height of the band it fills, where the offline z₀ takes the run's bottom slot. That is a
    constant offset over uniform fluid, so it drops out of ∇Υˡ and reaches Π_A only near the walls.
...
def online_name(ℓ, suffix=""):
    """Online output variable for scale ℓ, matching the Julia Symbol("Π_A_ℓ$(ℓ)")."""
    tag = f"{int(ℓ)}" if float(ℓ) == int(ℓ) else f"{ℓ}"
    return f"Π_A_ℓ{tag}{suffix}"
#---
    ...
    for name in (online_name(ℓ), online_name(ℓ, "_int")):
        if name not in ds:
            raise SystemExit(f"Online field '{name}' not in {filename} — rerun the simulation with "
                             ...
                             f"`AvailablePotentialEnergyCrossScaleFlux`).")
...
for row, ℓ in enumerate(args.filter_scales):
    ...
    offline = calculate_cross_scale_ape_flux(ds_rho["ρ"], ds_rho["uᵢ"], filt_local_pes.upsilon,
                                             gf, filter_dims=FILTER_DIMS,
                                             filtered_density=ds_filt["ρ̄"],
                                             filtered_velocity_vector=ūᵢ).rename("Π_A")
    ...
    check(rms_field, f"    Π_A field (ℓ={ℓ:g}): rms(diff)/rms(online) = {rms_field:.3e},  "
    ...
    check(rms_int, f"    ∫Π_A dV (ℓ={ℓ:g}): rms(online - offline)/rms(offline) = {rms_int:.3e}", print)
...
ax.set(xlabel="time", ylabel="∫Π_A dV", title="Volume-integrated cross-scale APE flux")
ax.legend(fontsize=9)
ax.grid(alpha=0.3)

fig.suptitle(f"Online vs offline Π_A   maps at t = {t_sel:.1f}, volume integrals over the run"
             + (f"   {label}" if label else ""))
out = FIGURES / f"inv09_ape_transfer_{stem}_t{t_sel:.1f}.png"
