...
ℓ = float(a.filter_scale.sel(filter_scale=args.filter_scale, method="nearest").values)
a = a.sel(filter_scale=ℓ, method="nearest")
b = b.sel(filter_scale=args.filter_scale, method="nearest")
...
for v in [x for x in a.data_vars if x.startswith("∫")]:
    if v not in b:
        continue
    A = np.asarray(a[v].values, dtype=float).ravel()
    B = np.asarray(b[v].values, dtype=float).ravel()
    mA, mB = A.mean(), B.mean()
