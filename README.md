...
also emits the online local available potential energy `E_a` (and its integral `∫E_a`), the sub-filter
APE `E_as_ℓ<ℓ>`, and the sub-filter APE dissipation `ε_As_ℓ<ℓ>` at each online filter scale, the last of
which `05_sfs_ape_budget.py` reads back instead of recomputing. All of these
...
computation produce; `inv06_compare_sorted_profiles.py`, `inv07_compare_local_ape.py`,
`inv08_compare_sfs_ape_dissipation.py` and `inv10_compare_sfs_ape.py` compare the two. Each method carries its own full-domain sort per
...
compares them against the simulation's online diagnostics (`postprocessing/validation/inv01`–`inv10`),
writing comparison figures to `figures/validation/` and online-vs-offline animations to `animations/`. Note that
`inv06`, `inv07`, `inv08` and `inv10` need a run with `SAVE_SORTED=1` (which `submit_all_pbs.sh VALIDATE=1` sets automatically).
